...
RSpec.describe "flashcards" do
  context "with the same word on multiple lists" do
    ...
    it "adds the word only once" do
      expect(user.first_flashcard_list.words).to be_empty

      Flashcards.add_list(learning_group, list1)
      expect(user.first_flashcard_list.words).to match_array [noun1, noun2]
    ...
    it "does not add the word when it is already in another flashcard section" do
      ...
      expect(user.first_flashcard_list.words).to be_empty
      expect(user.flashcard_list(Flashcards::SECTIONS.second).words).to match_array [noun1]
      ...
      Flashcards.add_list(learning_group, list1)
      expect(user.first_flashcard_list.words).to match_array [noun2]
      expect(user.flashcard_list(Flashcards::SECTIONS.second).words).to match_array [noun1]
    end
