...
class Import::WordTest < ActiveSupport::TestCase
  ...
  setup do
    ...
    @topic = "Tiere"
    @word_type = "Noun"
    @word_import_id = create(:word_import, name: @name, topic: @topic, word_type: @word_type, state: :new).id
  ...
  test "with an empty database, calls the LLM" do
    name = @name
    topic = @topic
    word_type = @word_type

    new_word = NewWord.new(word_type: word_type, name: name, topic: topic)
    constructor_args = nil

    original_new = Llm::CheckBaseForm.method(:new)
    Llm::CheckBaseForm.define_singleton_method(:new) do |**kwargs|
      constructor_args = kwargs
      instance = original_new.call(**kwargs)
      instance.define_singleton_method(:call) { new_word }
      instance
    end

    begin
      Import::Word.new(name: name, topic: topic, word_type: word_type, word_import_id: @word_import_id).call
    ensure
      Llm::CheckBaseForm.singleton_class.send(:remove_method, :new)
    end

    assert_equal({word_type: word_type, name: name, topic: topic}, constructor_args)

    last = WordImport.last
    assert_equal name, last.name
    assert_equal topic, last.topic
    assert_equal word_type, last.word_type
    assert_equal "completed", last.state
  end

  test "with the same word, enriches the word" do
    word = create(:noun, name: @name, topics: [build(:topic, name: @topic)])

    constructor_args = nil
    original_new = Llm::Enrich.method(:new)
    Llm::Enrich.define_singleton_method(:new) do |**kwargs|
      constructor_args = kwargs
      instance = original_new.call(**kwargs)
      instance.define_singleton_method(:call) { nil }
      instance
    end
    ...
    begin
      ...
      Llm::Enrich.singleton_class.send(:remove_method, :new)
    end

    assert_equal({word: word}, constructor_args)
  ...
  test "with unlisted keywords, processes unlisted keywords when word already exists" do
    word = create(:noun, name: @name, topics: [build(:topic, name: @topic)])
    original_word = create(:noun, name: "Adler", topics: [build(:topic, name: "Tiere")])
    unlisted_keyword = create(:unlisted_keyword, word: original_word, word_import: WordImport.last, state: "new")
    ...
    initial_keyword_count = original_word.keywords.count
    begin
      Import::Word.new(name: @name, topic: @topic, word_type: @word_type, word_import_id: @word_import_id).call
    ensure
      Llm::Enrich.singleton_class.send(:remove_method, :new)
    ...
    assert_equal initial_keyword_count + 1, original_word.reload.keywords.count
    assert_equal "processed", unlisted_keyword.reload.state
    assert_includes original_word.keywords, word
  ...
  test "with a new word, creates a new word" do
    ...
    assert_equal @name, last.name
    assert_equal @topic, last.topic
    assert_equal "LLM Name", last.llm_name
    assert_equal "LLM Topic", last.llm_topic
  end
end
